...
##### MSM in Nhalangano with binomial prior for N  #####################
...
M_size=4038 # Total male population in Nhalangano 
...
# Generating phi #
phi[1]=rbeta(1,N_initial+a_phi,M_size+b_phi-N_initial)    #initial value of N=150 assumed


# Generating population size #

w=rbinom(1,M_size-NN,((phi[1]*p_000[1])/(1-phi[1]+(phi[1]*p_000[1]))))
N_estimate[1]=w+NN
...
for(h in 2: (tot+1)){
  ...
  # Generating phi #
  phi[h]=rbeta(1,N_estimate[h-1]+a_phi,M_size+b_phi-N_estimate[h-1])  
  
  # Generating population size #
  
  p_000[h]=(1-beta_0[h])*(1-p[1,h])*(1-p[2,h])*(1-p[3,h])
  
  w=rbinom(1,M_size-NN,((phi[h]*p_000[h])/(1-phi[h]+(phi[h]*p_000[h]))))
  N_estimate[h]=w+NN
